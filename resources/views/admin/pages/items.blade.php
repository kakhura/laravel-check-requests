...
@section('title', 'დინამიური გვერდები')

@section('content')
    <div class="page-title">
        <div class="title_left">
            <h3>დინამიური გვერდები</h3>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="x_panel">
                <div class="x_title">
                    <a href="{{url('admin/pages/create')}}" class="btn btn-sm btn-success pull-right">
                ...
                <div class="x_content admin_container">
                    @if(count($pages))
                            ...
                            <tbody>
                                @foreach($pages as $key => $page)
                                    <tr class="cursor-move">
                                        <td class="text-center sort" id="sort{{ $key }}" data-id="{{ $page->id }}" data-ordering ="{{ $page->ordering }}">
                                            {{ $page->ordering }}
                                        </td>
                                        <td class="text-center">
                                            <input type="checkbox" value="published" id="{{ $page->id }}" class="js-switch publish" {{ $page->published ? 'checked' : '' }} />
                                        </td>
                                        <td>
                                            @if($page->image)
                                                <img class="post" src="{{ asset($page->image) }}" alt="" />
                                            @endif
                                            <a href="{{ url('admin/pages/edit/' . $page->id) }}">
                                                {{ Str::limit($page->detail()->where('locale', 'ka')->first()->title, 35) }}
                                            </a>
                                        </td>
                                        <td class="text-center">
                                        ...
                                        <td align="right">
                                            <a href="{{ url('admin/pages/edit/' . $page->id) }}" class="btn btn-info btn-sm"><i class="fa fa-pencil"></i> </a>

                                            <a href="{{ url('admin/pages/delete/' . $page->id) }}" class="delete btn btn-danger btn-sm"><i class="fa fa-trash-o"></i></a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                        ...
                        {{ $pages->links() }}
                    @else
                        <div class="alert alert-info">დინამიური გვერდები ვერ მოიძებნა</div>
                    @endif
                </div>
            </div>
        </div>
    ...
    @if(isset($limit))
        <script>
            ...
            function changeordering(){
                ...
                $.ajax({
                    url:"{{ url('admin/pages/ordering') }}",
                    type:"POST",
                    data:"_token={{ csrf_token() }}" + "&ordering=" + arr + "&className={{ addslashes(config('kakhura.site-bases.ordering_classes.pages')) }}",
                }).done(function(data){})
            }
        </script>
    @endif
        ...
        $(document).ready(function() {
            ...
            $('.publish').change(function(event) {
                ...
                $.ajax({
                    ...
                    url: '{{ url("admin/pages/publish") }}',
                    type: "post",
                    data: {
                        id: id,
                        published: published,
                        _token: '{{ csrf_token() }}',
                        className: "{{ addslashes(config('kakhura.site-bases.publish_classes.pages')) }}"
                    },
