...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="x_panel">
                <div class="x_title">
                    <a href="{{url('admin/services/create')}}" class="btn btn-sm btn-success pull-right">
                ...
                <div class="x_content admin_container">
                    @if(count($services))
                            ...
                            <tbody>
                                @foreach($services as $key => $service)
                                    <tr class="cursor-move">
                                        <td class="text-center sort" id="sort{{ $key }}" data-id="{{ $service->id }}" data-ordering ="{{ $service->ordering }}">
                                            {{ $service->ordering }}
                                        </td>
                                        <td class="text-center">
                                            <input type="checkbox" value="published" id="{{ $service->id }}" class="js-switch publish" {{ $service->published ? 'checked' : '' }} />
                                        </td>
                                        <td>
                                            @if($service->image)
                                        ...
                                        <td align="right">
                                            <a href="{{ url('admin/services/edit/' . $service->id) }}" class="btn btn-info btn-sm"><i class="fa fa-pencil"></i> </a>

                                            <a href="{{ url('admin/services/delete/' . $service->id) }}" class="delete btn btn-danger btn-sm"><i class="fa fa-trash-o"></i></a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                        ...
                        {{ $services->links() }}
                    @else
                        <div class="alert alert-info">სერვისები ვერ მოიძებნა</div>
                    @endif
                </div>
            </div>
        </div>
    ...
    @if(isset($limit))
        <script>
            ...
            function changeordering(){
                ...
                $.ajax({
                    url:"{{ url('admin/services/ordering') }}",
                    type:"POST",
                    data:"_token={{ csrf_token() }}" + "&ordering=" + arr + "&className={{ addslashes(config('kakhura.site-bases.ordering_classes.services')) }}",
                }).done(function(data){})
            }
        </script>
    @endif
        ...
        $(document).ready(function() {
            ...
            $('.publish').change(function(event) {
                ...
                $.ajax({
                    ...
                    url: '{{ url("admin/services/publish") }}',
                    type: "post",
                    data: {
                        id: id,
                        published: published,
                        _token: '{{ csrf_token() }}',
                        className: "{{ addslashes(config('kakhura.site-bases.publish_classes.services')) }}"
                    },
