...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="x_panel">
                <div class="x_title">
                    <a href="{{url('admin/products/create')}}" class="btn btn-sm btn-success pull-right">
                ...
                <div class="x_content admin_container">
                    @if(count($products))
                            ...
                            <tbody>
                                @foreach($products as $key => $product)
                                    <tr class="cursor-move">
                                        <td class="text-center sort" id="sort{{ $key }}" data-id="{{ $product->id }}" data-ordering ="{{ $product->ordering }}">
                                            {{ $product->ordering }}
                                        </td>
                                        <td class="text-center">
                                            <input type="checkbox" value="published" id="{{ $product->id }}" class="js-switch publish" {{ $product->published ? 'checked' : '' }} />
                                        </td>
                                        <td>
                                            @if($product->image)
                                                <img class="post" src="{{ asset($product->image) }}" alt="" />
                                            @endif
                                            <a href="{{ url('admin/products/edit/' . $product->id) }}">
                                                {{ Str::limit($product->detail()->where('locale', 'ka')->first()->title, 35) }}
                                            </a>
                                        </td>
                                        <td class="text-center">
                                        ...
                                        <td align="right">
                                            <a href="{{ url('admin/products/edit/' . $product->id) }}" class="btn btn-info btn-sm"><i class="fa fa-pencil"></i> </a>

                                            <a href="{{ url('admin/products/delete/' . $product->id) }}" class="delete btn btn-danger btn-sm"><i class="fa fa-trash-o"></i></a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                        ...
                        {{ $products->links() }}
                    @else
                        <div class="alert alert-info">პროდუქცია ვერ მოიძებნა</div>
                    @endif
                </div>
            </div>
        </div>
    ...
    @if(isset($limit))
        <script>
            ...
            function changeordering(){
                ...
                $.ajax({
                    url:"{{ url('admin/products/ordering') }}",
                    type:"POST",
                    data:"_token={{ csrf_token() }}" + "&ordering=" + arr + "&className={{ addslashes(config('kakhura.site-bases.ordering_classes.products')) }}",
                }).done(function(data){})
            }
        </script>
    @endif
        ...
        $(document).ready(function() {
            ...
            $('.publish').change(function(event) {
                ...
                $.ajax({
                    ...
                    url: '{{ url("admin/products/publish") }}',
                    type: "post",
                    data: {
                        id: id,
                        published: published,
                        _token: '{{ csrf_token() }}',
                        className: "{{ addslashes(config('kakhura.site-bases.publish_classes.products')) }}"
                    },
