...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="x_panel">
                <div class="x_title">
                    <a href="{{url('admin/partners/create')}}" class="btn btn-sm btn-success pull-right">
                ...
                <div class="x_content admin_container">
                    @if(count($partners))
                            ...
                            <tbody>
                                @foreach($partners as $key => $partner)
                                    <tr class="cursor-move">
                                        <td class="text-center sort" id="sort{{ $key }}" data-id="{{ $partner->id }}" data-ordering ="{{ $partner->ordering }}">
                                            {{ $partner->ordering }}
                                        </td>
                                        <td class="text-center">
                                            <input type="checkbox" value="published" id="{{ $partner->id }}" class="js-switch publish" {{ $partner->published ? 'checked' : '' }} />
                                        </td>
                                        <td>
                                            @if($partner->image)
                                                <img class="post" src="{{ asset($partner->image) }}" alt="" />
                                            @endif
                                            <a href="{{ url('admin/partners/edit/' . $partner->id) }}">
                                                {{ Str::limit($partner->detail()->where('locale', 'ka')->first()->title, 35) }}
                                            </a>
                                        </td>
                                        <td class="text-center">
                                        ...
                                        <td align="right">
                                            <a href="{{ url('admin/partners/edit/' . $partner->id) }}" class="btn btn-info btn-sm"><i class="fa fa-pencil"></i> </a>

                                            <a href="{{ url('admin/partners/delete/' . $partner->id) }}" class="delete btn btn-danger btn-sm"><i class="fa fa-trash-o"></i></a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                        ...
                        {{ $partners->links() }}
                    @else
                        <div class="alert alert-info">პარტნიორები ვერ მოიძებნა</div>
                    @endif
                </div>
            </div>
        </div>
    ...
    @if(isset($limit))
        <script>
            ...
            function changeordering(){
                ...
                $.ajax({
                    url:"{{ url('admin/partners/ordering') }}",
                    type:"POST",
                    data:"_token={{ csrf_token() }}" + "&ordering=" + arr + "&className={{ addslashes(config('kakhura.site-bases.ordering_classes.partners')) }}",
                }).done(function(data){})
            }
        </script>
    @endif
        ...
        $(document).ready(function() {
            ...
            $('.publish').change(function(event) {
                ...
                $.ajax({
                    ...
                    url: '{{ url("admin/partners/publish") }}',
                    type: "post",
                    data: {
                        id: id,
                        published: published,
                        _token: '{{ csrf_token() }}',
                        className: "{{ addslashes(config('kakhura.site-bases.publish_classes.partners')) }}"
                    },
