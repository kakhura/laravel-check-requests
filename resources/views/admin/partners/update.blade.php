...
@section('title', $partner->detail()->where('locale', config('kakhura.site-bases.admin_editors_default_locale'))->first()->title)

@section('content')
    @include('vendor.site-bases.admin.inc.message')
    <div class="page-title">
        <div class="title_left">
            <h3>{{ $partner->detail()->where('locale', config('kakhura.site-bases.admin_editors_default_locale'))->first()->title }}</h3>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
                    ...
                    <div class="col-md-10 col-sm-12">
                        <form method="POST" action="" data-parsley-validate class="form-horizontal"  enctype="multipart/form-data">
                                    ...
                                    <div class="tab-pane {{ $localeCode == config('kakhura.site-bases.admin_editors_default_locale') ? 'active' : '' }}" id="{{ $localeCode }}">
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-10 col-sm-10  col-xs-12">
                                                <input type="text" name="title_{{ $localeCode }}" class="form-control" id="title_{{ $localeCode }}" value="{{ $partner->detail()->where('locale', $localeCode)->first()->title }}" required>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-10 col-sm-10 col-xs-12">
                                                <textarea id="description_{{ $localeCode }}" class="textarea" name="description_{{ $localeCode }}">{{ $partner->detail()->where('locale', $localeCode)->first()->description }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-10 col-sm-10  col-xs-12">
                                    <input type="text" name="video" class="form-control" id="video" value="{{ $partner->video }}">
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <div class="col-md-offset-2 col-md-3 col-sm-4 col-xs-12">
                                    <label>
                                        <input type="checkbox" name="published" class="js-switch" {{ $partner->published ? 'checked' : '' }} />
                                        გამოქვეყნებულია
                                    </label>
                                </div>
                            </div>
